...
#define MAXBYTES 6000

bool checkTag(const char* tag, const char *in, const char *context, int argc, char **argv, FILE *newFile)
{
    char buffer[100];
    ...
    if (!strncmp(in, buffer, strlen(buffer))) {
		...
		fprintf(newFile, "%s\n", in);
                ...
                // Prepare the lookup string: tag + '|' + context + 0x04 + text
                char lookup[MAXBYTES];
                int lpos = 0;
                // tag
                strncpy( lookup, tag, MAXBYTES );
                lpos += strlen( tag );
                // context
                if ( context && context[0] ) {
                        lookup[lpos] = '|';
                        lpos += 1;
                        strncpy( lookup + lpos, context, MAXBYTES - lpos );
                        lpos += strlen( context );
                        if ( lpos + 1 >= MAXBYTES ) { // +1 for the separator below
                                fprintf( stderr, "Internal error: context string too long!" );
                                return false;
                        }
                }
                // context separator: Gettext separator for context is 0x04
                lookup[lpos] = 4;
                lpos += 1;
                // the text proper
                const char* separator = strchr( in, '=' );
                if ( !separator ) {
                        fprintf( stderr, "Internal error: cannot find '=' again!" );
                        return false;
                }
                strncpy( lookup + lpos, separator + 1, MAXBYTES - lpos );
                lpos += strlen( separator + 1 );
                if ( lpos >= MAXBYTES ) {
                        fprintf( stderr, "Internal error: lookup string too long!" );
                        return false;
                }
		...
		for (int i = 2; i < argc; i++) {
			...
			const char *test = k_dcgettext(domain, lookup, "abc");
                        ...
                        if ( test == lookup ) {
                                ...
                                // We have a 4 char but not the original translation: what is happening here?
			...
			if (strchr(test, '\n')) {
				...
				char out[MAXBYTES];
				sprintf(out, "%s[%s]=%s", tag, argv[i], test);
				fprintf(newFile, "%s\n", out);
			}
			return true;
		}
...
bool checkContext (const char *line, char *context, FILE *file)
{
    static const char *ctxtkey = "ctxt";
    static char ctxtsep = ':';

    const char *lhead = line;

    #define NEXTNONWS \
    while (*lhead && (*lhead == ' ' || *lhead == '\t')) { \
        ++lhead; \
    }
    NEXTNONWS
    if (*lhead != '#') {
    ...
    NEXTNONWS
    int i = 0;
    while (*lhead && ctxtkey[i] && *lhead == ctxtkey[i++]) {
        ++lhead;
    }
    if (ctxtkey[i]) { // context keyword not fully matched
        return false;
    }
    NEXTNONWS
    if (*lhead != ctxtsep) {
    ...
    // We have a context (line has had trailing whitespace removed).
    strncpy(context, lhead, MAXBYTES);
    context[MAXBYTES - 1] = 0;
    fprintf(file, "%s\n", line);
    return true;
}
...
int main(int argc, char **argv)
{
    ...
    char in[MAXBYTES];
    char context[MAXBYTES];
    bool afterContext;

    context[0] = 0;
    afterContext = false;

    while (fgets(in, MAXBYTES, f) != 0) {
	...
	while (in[len] == ' ' || in[len] == '\t')
	   in[len--] = 0;
        ...
        // context is valid only for the immediate next line
        if ( checkContext( in, context, newFile ) ) {
            afterContext = true;
            continue;
        }
        if ( !afterContext && context[0] ) {
            context[0] = 0;
        }
        afterContext = false;

	if (checkTag("Name", in, context, argc, argv, newFile))
	    continue;

	if (checkTag("Comment", in, context, argc, argv, newFile))
	    continue;

	if (checkTag("Language", in, context, argc, argv, newFile))
	    continue;

	if (checkTag("Keywords", in, context, argc, argv, newFile))
	    continue;

	if (checkTag("X-KDE-Keywords", in, context, argc, argv, newFile))
	    continue;

	if (checkTag("About", in, context, argc, argv, newFile))
	    continue;

	if (checkTag("Description", in, context, argc, argv, newFile))
	    continue;

	if (checkTag("GenericName", in, context, argc, argv, newFile))
	    continue;

	if (checkTag("Query", in, context, argc, argv, newFile))
	    continue;

	if (checkTag("ExtraNames", in, context, argc, argv, newFile))
	    continue;

        // ### TODO: KDE4: do we still need it?
	if (checkTag("X-KDE-Submenu", in, context, argc, argv, newFile))
	    continue;
